...
Use the high-level upload workflow. It computes the content hash before init,
prefers a direct presigned object-store upload, and commits only after a
successful transfer:
...
`source` accepts a `Uint8Array`, `ArrayBuffer`, `Blob`, `{ path: string }`, or
a reopenable source with a known size and SHA-256. An unknown one-shot stream is
intentionally rejected: the server pins size and hash during init, before any
bytes are transferred. Set `uploadPreference: "proxy"` only when the server is
configured to allow its bounded multipart fallback.
...
```typescript
const response = await client.doc.download(...);
...
```typescript
import { DeploymentClient } from '@cloudpdf/sdk/deployment';
...
```typescript
const { data, rawResponse } = await client.shares.exchange(...).withRawResponse();
