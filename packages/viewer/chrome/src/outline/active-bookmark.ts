import type { OutlineBookmark, OutlineDestination } from './types';

function isGotoActionType(type: unknown): boolean {
  return type === 1 || type === 2 || type === 'goto' || type === 'goto-remote';
}
...
export function resolveBookmarkDestination(
  ...
  if (target.type === 'action') {
    const { action } = target;
    if (isGotoActionType(action.type) && 'destination' in action) {
 ...
 * navigable page.
 */
export function resolveBookmarkPageIndex(bookmark: OutlineBookmark): number | undefined {
...
export function getActiveBookmarkPath(
  ...
  const walk = (nodes: readonly OutlineBookmark[], prefix: number[]): void => {
