...
import type { SearchLiteralQuery } from './types';
 ...
 */
export function foldOptionsFor(query: SearchLiteralQuery): FoldOptions {
...
function wordBefore(folded: string, index: number): boolean {
  if (index <= 0) return false;
  // Step back over a low surrogate to test the full code point.
  const unit = folded.charCodeAt(index - 1);
  const start = unit >= 0xdc00 && unit <= 0xdfff && index >= 2 ? index - 2 : index - 1;
  return WORD_UNIT.test(String.fromCodePoint(folded.codePointAt(start)!));
}

/** Whether the code point starting at `index` is a word char. */
function wordAt(folded: string, index: number): boolean {
  if (index >= folded.length) return false;
  return WORD_UNIT.test(String.fromCodePoint(folded.codePointAt(index)!));
}
 ...
 */
export function matchLiteral(haystack: FoldedText, query: SearchLiteralQuery): SearchMatchRange[] {
