 ...
 */
describe('positionAnchoredRect — knob nudges only the placement edge', () => {
  // A 100×40 box at (100, 100): centre (150, 120).
  const box = { x: 100, y: 100, width: 100, height: 40 };
  const gap = 8;

  describe("placement 'top'", () => {
    const noKnob = positionAnchoredRect(box, 'top', gap);
  ...
  describe("placement 'bottom'", () => {
    const noKnob = positionAnchoredRect(box, 'bottom', gap);
  ...
  describe("placement 'left'", () => {
    const noKnob = positionAnchoredRect(box, 'left', gap);
  ...
  describe("placement 'right'", () => {
    const noKnob = positionAnchoredRect(box, 'right', gap);
