...
import { useInTabs } from './tabs.client';
...
export function Pre({ children, className, 'data-filename': filename, ...props }: PreProps) {
  ...
  // code-block box owns the chrome (tab bar + copy button).
  if (inTabs) {
    return (
      <pre
        ref={preRef}
        className={`dk-dark-scroll m-0 overflow-x-auto px-[18px] py-[17px] font-mono text-[13px] leading-[1.8] text-[#C8D3EA] [&_code]:bg-transparent [&_code]:p-0 ${className ?? ''}`}
        {...props}
      >
        {children}
      </pre>
