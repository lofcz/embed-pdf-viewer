...
const styles: Record<CalloutType, { box: string; icon: string; body: string }> = {
  info: {
    ...
    icon: 'bg-[#DCEAFF] text-[var(--dk-accent)]',
    body: 'text-[#2A4574] [&_b]:text-[var(--dk-heading)] [&_strong]:text-[var(--dk-heading)]',
  },
  warn: {
...
export function Callout({ type = 'info', children }: { type?: CalloutType; children: ReactNode }) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`font-sans text-[15px] leading-[1.6] ${s.body} [&_a]:text-[var(--dk-accent)] [&>:first-child]:mt-0 [&_a:hover]:underline [&_a]:font-semibold`}
      >
        {children}
      </div>
    </div>
