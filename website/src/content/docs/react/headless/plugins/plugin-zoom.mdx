...
The `useZoom` hook is the primary way to interact with the plugin. It accepts a `documentId` and returns the current zoom `state` and a `provides` object with methods to change the zoom for that document.
...
export const ZoomToolbar = ({ documentId }) => {
...
To enable this, place the `<MarqueeZoom />` component inside the `renderPage` prop of your `<Scroller />`. It must be wrapped in a `PagePointerProvider`.
...
import { PDFViewer } from '../../code-examples/headless/zoom-example'

<CodeExample 
  codePaths={[
    "content/docs/react/code-examples/headless/zoom-example.tsx"
  ]}
>
  <PDFViewer />
</CodeExample>
...
### Hook: `useZoom(documentId)`

This hook connects your component to the zoom plugin's state and functions for a specific document.
