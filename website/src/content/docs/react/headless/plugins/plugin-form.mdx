...
### 6. Handling Signature Field Clicks

Unsigned signature widgets are clickable in fill mode. When a user clicks one, the Form Plugin emits `onSignatureFieldRequest`, giving you the widget metadata plus a bound `request.sign(...)` helper. This keeps the UI flow in your app while leaving the low-level PDF preparation inside the engine.

```tsx
import {
  useFormCapability,
  type SignatureFieldRequest,
...
const SignatureFlow = ({ documentId }: { documentId: string }) => {
  const { provides: formCapability } = useFormCapability();
  ...
  useEffect(() => {
    ...
    return scope.onSignatureFieldRequest((request: SignatureFieldRequest) => {
      void (async () => {
        const signedPdf = await request.sign({
          subFilter: PdfSignatureSubFilter.PKCS7_DETACHED,
          contentsSize: 16384,
          signer: {
            async sign(digest, algorithm: PdfSignatureHashAlgorithm) {
              ...
              const { cmsBase64 } = await response.json();
              return base64ToArrayBuffer(cmsBase64);
            },
          },
        });

        downloadPdf(signedPdf, 'signed-form.pdf');
      })();
    });
...
<Callout type="warning" title="The backend must return CMS, not a raw RSA or ECDSA signature">
  PDF signature fields expect the `Contents` placeholder to contain a detached CMS/PKCS#7 object. Returning only the bare cryptographic signature bytes is not enough. The worker computes the PDF digest, but your signer callback must still return full CMS bytes.
</Callout>

<Callout type="info" title="Why this API is event-based">
  We intentionally keep the signing UX outside plugin config. Some apps want to call a backend immediately, others need a modal, identity picker, smart-card prompt, or approval step first. `onSignatureFieldRequest` gives you a clean handoff point for that app-specific flow.
</Callout>
...
## Live Example — Backend Digital Signing

This example shows the recommended first-release developer experience: the client listens for `onSignatureFieldRequest`, sends the digest to a backend route, receives detached CMS bytes, and downloads the signed PDF. The example backend generates a self-signed certificate on the server, so Adobe will mark the signer as untrusted by default, but the PDF is still cryptographically signed correctly.

import { FormSigningViewer } from '../../code-examples/headless/form-signing-example';

<CodeExample
  codePaths={[
    "content/docs/react/code-examples/headless/form-signing-example.tsx",
    "app/api/examples/pdf-signing/route.ts"
  ]}
>
  <FormSigningViewer />
</CodeExample>

<Callout type="info" title="What runs on the client vs. the backend">
  The client should own user interaction and call `request.sign(...)`. The backend should own private keys and produce the detached CMS response. That split gives the best default security model without making the UI API rigid.
</Callout>
...
- **Signature Field** — Clickable in fill mode for unsigned widgets, with signing delegated through `onSignatureFieldRequest`
...
| **`requestSignatureField(annotationId, documentId?)`** | Manually triggers the same signing request flow used by clickable signature widgets. |
...
| **`onSignatureFieldRequest`** | `(request: SignatureFieldRequest) => void` | Fires when an unsigned signature widget is requested by the user or manually via `requestSignatureField`. |
...
Passed to the `onSignatureFieldRequest` callback.

```typescript
interface SignatureFieldRequest {
  documentId: string;
  pageIndex: number;
  annotationId: string;
