...
### Important: FPDF_InitLibrary()

The call to `FPDF_InitLibrary()` is critical and must be done before performing any PDF operations. This function initializes the PDFium library and allocates global resources. Failing to call this function may result in unexpected behavior or crashes.

## Basic Workflow

Working with PDFs in PDFium typically follows this pattern:

1. Initialize PDFium (including calling `FPDF_InitLibrary()`)
