...
### FPDF_InitLibrary Initialization

The `FPDF_InitLibrary()` function call is a crucial step in the initialization process. This is a standard PDFium function that allocates global resources required by the library. It must be called after the WebAssembly module is initialized but before any PDF operations are performed.
...
1. Initialize PDFium (with FPDF_InitLibrary)
2. Load a document
...
1. **Always call FPDF_InitLibrary**: Call this function once after initializing the WebAssembly module and before any PDF operations.
