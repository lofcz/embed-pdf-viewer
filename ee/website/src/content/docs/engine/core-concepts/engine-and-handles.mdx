...
`createCloudEngine(options)` returns an object implementing the `Engine`
interface:
...
`open()` is the only entry point. The cloud engine accepts two input kinds —
`{ kind: 'token' }` and `{ kind: 'id' }` (see
[Authentication](/docs/engine/getting-started/authentication)). Call
`destroy()` when your app is shutting the engine down; existing handles should
be `close()`d first.
...
```ts
const engine = createCloudEngine({ baseUrl, token });
